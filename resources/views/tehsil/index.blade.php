            ...
            <div class="card mb-4">
                <div class="card-header pb-0 d-flex justify-content-between align-items-center category-card-header">
                    <div class="category-card-header-top">
                        <h6 class="m-0">Tehsils</h6>

                        <a href="{{ route('tehsil.add') }}" class="btn btn-primary btn-sm category-card-add-btn">
                            + Add
                        </a>
                    </div>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                                ...
                                @foreach ($tehsil as $key => $item)
                                    <tr data-id="{{ $item->id }}">
                                        <td>
                                            ...
                                            {{ $tehsil->firstItem() + $key }}
                                        </td>

                                        <td><p class="text-xs font-weight-bold mb-0">{{ $item->district_name }}</p></td>
                                        <td><p class="text-xs font-weight-bold mb-0">{{ $item->name }}</p></td>

                                        <td>
                                            <a href="{{ route('tehsil.edit', $item->id) }}"
                      ...
                      <div class="mt-4">
                            {{ $tehsil->links('shared.pagination') }}
                        </div>
                    </div>
                </div>
            </div>
